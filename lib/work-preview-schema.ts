...
export function workPreviewInstructions(capturedAt:string,timeZone:string){
  return [
    ...
    'For updates, output the COMPLETE desired case state. Preserve every existing field that the user did not change, including closing date and follow-up. Null means the final value should be empty, not “unchanged.”',
    ...
    'Use event_kind=request when Dylan made a request; response when something came back; action for Dylan’s completed step; follow_up when Dylan circled back; completion when the case closed; note/status for other factual changes.',
    'The confirmation_question is what the wizard will show with Yes and No buttons. Make it concrete and complete enough that Dylan can see exactly where Orbit intends to put the information. Example: “Put G26-0441 in Waiting on Response, with the ball with Mike for the LLC documents, and follow up Monday — is that right?”',
