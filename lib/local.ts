...
export async function putLocal(store:Store,value:unknown) {
  ...
  await new Promise<void>((resolve,reject)=>{
    ...
    tx.oncomplete=()=>resolve();tx.onerror=tx.onabort=()=>reject(new Error('Could not save on this device. Free up storage and try again.'));
  });
}
...
export async function capturesFor(userId:string){return(await allLocal<Capture>('captures')).filter(c=>c.user_id===userId);}
