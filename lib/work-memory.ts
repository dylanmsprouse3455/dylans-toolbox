...
export async function workMemoryContext(client:SupabaseClient,text:string,focusCaseId?:string){
  ...
  if(words.length){
    ...
    const facts=await client.from('work_case_facts').select('*').eq('active',true).or(filters.join(',')).order('updated_at',{ascending:false}).limit(120);
  ...
  return {
    ...
    input:{
      ...
      search_terms:words,
      existing_cases:selected.map(item=>({...item,history:grouped.get(item.id)??[],facts:factsByCase.get(item.id)??[],phases:phasesByCase.get(item.id)??[]})),
      matched_facts:matchedFacts.slice(0,50),
      matched_history:matchedEvents.slice(0,50),
      matched_prior_captures:captureHits.map(row=>({id:row.id,user:row.source_text.slice(0,3000),result:compactTurnResult(row.turn_result),created_at:row.created_at})),
