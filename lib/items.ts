...
export function priority(item: Item, now = Date.now()) {
  const hours = item.due_at ? (Date.parse(item.due_at) - now) / 3600000 : Infinity;
...
export function attention(items: Item[], now = Date.now()) {
  return items.filter(i => actionable(i) && i.status === 'active' && !i.parent_id &&
    (i.importance >= 4 || i.urgency >= 4 || (!!i.due_at && Date.parse(i.due_at) <= now + 72 * 3600000)))
...
export function dueLabel(date: string | null) {
  ...
  return d.toLocaleDateString([], {month:'short',day:'numeric',...(d.getFullYear() !== now.getFullYear() ? {year:'numeric' as const} : {})});
}
