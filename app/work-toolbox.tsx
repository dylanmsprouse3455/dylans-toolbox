...
export default function WorkToolbox(){
  ...
  useEffect(()=>{
    ...
    getSupabase().then(async client=>{
      ...
      const accept=async(next:Session|null)=>{
        ...
        sessionRef.current=next;setSession(next);setReady(true);if(next)void refresh();else setCases([]);
      };
  ...
  async function commitWizard(){
    ...
    try{
      ...
      const saved=result as WorkCommitReceipt;setWizard(null);setReply(saved.reply);speakReply(saved.reply);if(saved.case_ids[0])setFocusCaseId(saved.case_ids[0]);await refresh();if(selectedId&&saved.case_ids.includes(selectedId))await loadEvents(selectedId);
    }catch(e){setError(errorText(e));}finally{setBusy(false);}
  }
  ...
  return <main className="work-toolbox">
    ...
    <section className="work-board">
      {groups.map(group=>{const rows=active.filter(item=>item.workflow_state===group.state);const Icon=group.icon;return <section className="work-group" key={group.state}><div className="work-group-heading"><span><Icon/></span><div><h2>{group.label}</h2><p>{group.help}</p></div><b>{rows.length}</b></div>{rows.length?<div className="work-list">{rows.map(item=><button type="button" className="work-item" key={item.id} onClick={()=>openCase(item)}><div className="work-item-main"><h3>{item.case_number?item.case_number+' · ':''}{item.title.replace(/^G\d{2}-\d{4}\s*[·—-]?\s*/,'')}</h3>{item.current_situation&&<p>{item.current_situation}</p>}<div className="work-meta"><span>{ballLabel(item.ball_owner,item.ball_with)}</span>{followUpLabel(item)&&<span>{followUpLabel(item)}</span>}{item.closing_date&&<span>Close {dateLabel(item.closing_date)}</span>}</div></div><ChevronRight className="work-chevron"/></button>)}</div>:<p className="work-empty">Nothing here right now.</p>}</section>;})}
      <section className="work-group completed"><div className="work-group-heading"><span><CheckCircle2/></span><div><h2>Done</h2><p>Closed and completed Work files.</p></div><b>{completed.length}</b></div>{completed.length?<div className="work-list">{completed.slice(0,30).map(item=><button type="button" className="work-item" key={item.id} onClick={()=>openCase(item)}><div className="work-item-main"><h3>{item.case_number?item.case_number+' · ':''}{item.title.replace(/^G\d{2}-\d{4}\s*[·—-]?\s*/,'')}</h3>{item.current_situation&&<p>{item.current_situation}</p>}</div><ChevronRight className="work-chevron"/></button>)}</div>:<p className="work-empty">Nothing completed yet.</p>}</section>
      {loading&&<p className="work-loading"><LoaderCircle className="spinning"/> Refreshing Work…</p>}
    </section>

    {wizard&&currentProposal&&<div className="work-overlay" role="dialog" aria-modal="true" aria-label="Confirm Work update"><section className="work-wizard"><div className="work-wizard-top"><div><p className="work-step">Check {wizardIndex+1} of {wizard.preview.proposals.length}</p><h2>{wizard.preview.headline||'Check what Orbit understood'}</h2></div><Button size="icon" variant="ghost" aria-label="Cancel draft" onClick={()=>void discardWizard()} disabled={busy}><X/></Button></div><div className="work-wizard-file"><span className="work-case-badge">{currentProposal.case_number||'Work item'}</span><h3>{currentProposal.title}</h3><div className="work-wizard-chips"><span>{stateLabel[currentProposal.workflow_state]}</span><span>{ballLabel(currentProposal.ball_owner,currentProposal.ball_with)}</span>{currentProposal.status==='completed'&&<span>Completed</span>}</div></div><dl className="work-wizard-details"><div><dt>Current situation</dt><dd>{currentProposal.current_situation||'—'}</dd></div><div><dt>Next action</dt><dd>{currentProposal.next_action||'Nothing specific yet'}</dd></div><div><dt>When to care again</dt><dd>{dueLabel(currentProposal.follow_up_at,currentProposal.follow_up_date)||'No follow-up set'}</dd></div>{currentProposal.closing_date&&<div><dt>Closing</dt><dd>{dateLabel(currentProposal.closing_date)}</dd></div>}<div><dt>Timeline entry</dt><dd>{currentProposal.event_summary}</dd></div></dl><div className="work-wizard-question"><strong>{currentProposal.confirmation_question}</strong></div>{!correcting?<div className="work-wizard-actions"><Button onClick={()=>void confirmWizardStep()} disabled={busy}>{busy?<LoaderCircle className="spinning"/>:<><Check/>Yes, that’s right</>}</Button><Button variant="outline" onClick={()=>setCorrecting(true)} disabled={busy}><X/>No, change it</Button></div>:<div className="work-correction"><label>What did Orbit get wrong, or what should be different?<Textarea autoFocus value={correction} onChange={e=>setCorrection(e.target.value)} placeholder="Example: I’m waiting on Sarah, not Mike, and I don’t need to follow up until Tuesday." maxLength={5000}/></label><div className="work-wizard-actions"><Button onClick={()=>void reviseWizard()} disabled={busy||!correction.trim()}>{busy?<LoaderCircle className="spinning"/>:<><Send/>Fix the draft</>}</Button><Button variant="outline" onClick={()=>{setCorrecting(false);setCorrection('');}} disabled={busy}>Back</Button></div></div>}<p className="work-wizard-note">Nothing on this screen is saved until every check is confirmed.</p></section></div>}
