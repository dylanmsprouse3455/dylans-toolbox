...
import {AlertTriangle,BriefcaseBusiness,Check,CheckCircle2,ChevronRight,Clock3,Eye,History,LoaderCircle,Mic,PenLine,RotateCcw,Send,Sparkles,Square,X} from 'lucide-react';
...
export default function WorkToolbox(){
  ...
  const [approvedRuleKeys,setApprovedRuleKeys]=useState<string[]>([]),[ruleBusy,setRuleBusy]=useState<string|null>(null);
  ...
  async function approveRule(suggestion:WorkRuleSuggestion){
    ...
    try{
      const form=new FormData();form.set('mode','approve_rule');form.set('id',wizard.turn_id);form.set('rule_key',suggestion.rule_key);form.set('rule_text',suggestion.rule_text);
      const {response,result}=await workRequest(form);if(!response.ok)throw new Error(result.error||'Could not save that Work rule.');
      setApprovedRuleKeys(current=>[...current,suggestion.rule_key]);
    }catch(e){setError(errorText(e));}finally{setRuleBusy(null);}
  }
  ...
  async function discardWizard(){
    ...
    try{const form=new FormData();form.set('mode','discard');form.set('id',wizard.turn_id);await workRequest(form);setWizard(null);setCorrecting(false);setCorrection('');setApprovedRuleKeys([]);}
    catch(e){setError(errorText(e));}finally{setBusy(false);}
  }
