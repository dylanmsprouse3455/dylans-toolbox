...
Tap the microphone, speak, then tap again to save. The assistant describes what it actually created or changed and reads that reply aloud using the device's built-in voice; the written reply remains available with a replay button. A typed thought follows the same organization flow without unexpectedly speaking aloud. Captures save to IndexedDB before upload. A successful response is cached in the same transaction that removes its pending raw capture. Failure leaves the capture available for retry and playback. A server receipt prevents duplicate items after lost responses.
