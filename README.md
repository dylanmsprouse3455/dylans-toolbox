...
Open Talk, tap the microphone, speak, then tap again to save. When the browser supports live speech recognition, your words appear as provisional thought lines while you record so you can keep talking to correct or add details. The final AI organization still runs once after you finish. The assistant then describes what it actually created or changed, reads that reply aloud using the device's built-in voice, and shows the same reply in a compact corner chat bubble. A typed thought follows the same organization flow without unexpectedly speaking aloud. Captures save to IndexedDB before upload. A successful response is cached in the same transaction that removes its pending raw capture. Failure leaves the capture available for retry and playback. A server receipt prevents duplicate items after lost responses.
